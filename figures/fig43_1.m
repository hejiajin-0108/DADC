...
for n=1:3
Range(n)=max(X(n,:,N+1))-min(X(n,:,N+1));
StandardDeviation(n)=std(X(n,:,N+1))
InterquartileRange(n)=iqr(X(n,:,N+1));
end

%%%%%二维可视化
scatter(1:85,X(1,:,4001),150,[0 127 186]/255,marker='.');
hold on
scatter(1:85,X(2,:,4001),150,[217 103 36]/255,marker='.');
hold on
scatter(1:85,X(3,:,4001),150,[131 174 64]/255,marker='.');
hold on
xlabel('$i$','Interpreter','latex');
ylabel('$v_i$','Interpreter','latex');
legend('$\eta=0.6$',' $\eta=0.8$',' $\eta=1.0$','Interpreter','latex','fontsize',12);
